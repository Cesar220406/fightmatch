...
router.put('/:id', auth, requireRol('admin', 'editor'), async (req, res) => {
  ...
  try {
    const { rows } = await pool.query(
      `UPDATE lesiones SET nombre=$1, slug=$2, descripcion=$3, zona_corporal=$4, severidad=$5
       WHERE id=$6 RETURNING *`,
      [nombre, slug, descripcion, zona_corporal, severidad, req.params.id]
    );
...
router.delete('/:id', auth, requireRol('admin'), async (req, res) => {
  try {
    await pool.query('UPDATE lesiones SET activo=FALSE WHERE id=$1', [req.params.id]);
    res.json({ ok: true });
  } catch (err) {
    res.status(500).json({ error: 'Error interno' });
  }
